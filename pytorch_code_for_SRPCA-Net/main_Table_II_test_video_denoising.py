...
parser.add_argument('--train_data_name', type=str, default='Weizmann_trainCR20', help='training dataset name')
parser.add_argument('--test_data_name', type=str, default='Weizmann_testCR20', help='test dataset name')
...
num_train = 83 # number of training samples
num_test = 10  # number of test samples
n1 = 144 # size of a 3D tensor
n2 = 180 # size of a 3D tensor
n3 = 28 # size of a 3D tensor
...
with torch.no_grad():
    for data in seque_init:
        ...
        for chan in range(M_numpy.shape[2]):
            L_init_numpy[:, :, chan] = ndimage.median_filter(M_numpy[:, :, chan], size=(3, 3)) # initialization of the low-rank tensor for SRPCA-Net
